  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      <div className="center-stack">
        <Panel title="AI Build Command" action={<span className="online"><i /> AI available after authenticated request</span>}>
    ...
    <footer className="statusbar"><span><LockKeyhole size={14} /> {previewMode ? 'Read-only sample data' : 'Private account data'}</span><span><Database size={14} /> Database verified on successful refresh</span><span><RadioTower size={14} /> Refresh-based synchronization</span><span><Activity size={14} /> Dependency readiness requires sign-in</span><button onClick={previewMode ? signIn : refresh}><RefreshCw size={14} /> {previewMode ? 'Sign in' : 'Refresh workspace'}</button></footer>
...
function SystemsModal({ onClose }: { onClose: () => void }) {
  return <Modal title="Connected apps and deployment systems" onClose={onClose} wide><div className="systems-modal"><div className="verified-banner"><ShieldCheck size={18} /><span><strong>Registry audit snapshot · checked 2026-09-17</strong> AppDeploy runtime verified · GitHub repository access verified</span></div><div className="app-link-grid">{appDeployApps.map((app) => <a key={app.url} href={app.url} target="_blank" rel="noreferrer"><Cloud size={18} /><span><strong>{app.name}</strong><small>{app.detail}</small></span><ExternalLink size={14} /></a>)}</div><h3>Provider launch paths</h3><div className="provider-list">{deploymentTargets.map((target) => <a key={target.name} href={target.url} target="_blank" rel="noreferrer"><span>{target.name}<small>{target.detail}</small></span><StatusBadge value={target.status} /><ExternalLink size={14} /></a>)}</div></div></Modal>;
}
