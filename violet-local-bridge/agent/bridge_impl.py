...
def app_dir() -> Path:
    ...
    if os.name == 'nt':
        ...
        return Path(base) / 'VioletLocalBridge'
...
def battery_summary() -> str:
    if sys.platform == 'darwin':
        match = re.search(r'(\d+)%', run_text(['pmset', '-g', 'batt']))
...
def bluetooth_summary() -> dict[str, Any]:
    if sys.platform == 'darwin':
        ...
        state = re.search(r'State:\s*(\w+)', output)
        return {
            'bluetooth_state': state.group(1) if state else 'Unknown',
            'bluetooth_connected_devices': len(re.findall(r'Connected:\s*Yes', output, flags=re.I)),
        }
    if os.name == 'nt':
        output = run_text([
...
def discovery_report() -> dict[str, Any]:
    ...
    return {**bluetooth_summary(), 'lan_interfaces': interfaces[:5000], 'lan_neighbors': arp[:5000]}
...
def run_elevated_windows(command: str) -> tuple[int, str]:
    with tempfile.TemporaryDirectory(prefix='violet-admin-') as temp:
        ...
        output_ps = ps_escape(output_file)
        code_ps = ps_escape(code_file)
        script_ps = ps_escape(script)
        script.write_text(
        ...
        outer = (
            ...
            f"-ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-File','{script_ps}'); exit $p.ExitCode"
        )
...
def execute_command(config: dict[str, Any], command: dict[str, Any]) -> None:
    ...
    if kind == 'health':
        ...
        http_json(
            ...
            {'version': VERSION, 'health': health_report(), 'discovery': discovery},
            token=config['token'],
        )
        output = json.dumps(discovery, indent=2)
    elif kind == 'sync_files':
        output = json.dumps({'received': sync_inbox(config), 'sent': upload_outbox(config)}, indent=2)
    elif kind == 'shell':
                ...
                try:
                    ...
                    try:
                        http_json(
                            ...
                            'POST',
                            f"/api/bridge/commands/{command['id']}/result",
                            {'output': f'Agent error: {exc}', 'exitCode': 1},
                            token=config['token'],
                        )
                    except Exception:
